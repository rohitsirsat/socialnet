...
// A utility function to handle API request with loading, success and error handling
...
export const requestHandler = async (
  ...
  try {
    ...
    // Handle error cases, including unauthoruzed and forbidden cases
    if ([401, 403].includes(error?.response.data?.statusCode)) {
      localStorage.clear(); // Clear local storageon authentication issues
